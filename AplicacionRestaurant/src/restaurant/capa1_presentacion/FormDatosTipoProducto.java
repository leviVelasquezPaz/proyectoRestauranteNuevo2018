 ...
 * @author INGENIERIA
 */
public class FormDatosTipoProducto extends javax.swing.JDialog {

    /**
     * Creates new form FormDatosTipoProducto
     */
    private int x, y;
    ...
    public FormDatosTipoProducto(JDialog owner) throws Exception {
        super(owner, true);
        initComponents();
        ...
        txtCodigo.setText(String.valueOf(gestionarTipoProductoServicio.obtenerUltimoCodigo() + 1));
    }

    public FormDatosTipoProducto(JDialog owner, TipoProducto tipoProducto) throws Exception {
        super(owner, true);
        initComponents();
        RSAnimation.setMoverIzquierda(1050, 900, 15, 3, this);
        gestionarTipoProductoServicio = new GestionarTipoProductoServicio();
    ...
    private void llenarDatosTextos() {
        txtCodigo.setText(String.valueOf(tipoProducto.getTipoProductoCodigo()));
        txtNombre.setText(String.valueOf(tipoProducto.getTipoProductoNombre()));
        txtDescripcion.setText(String.valueOf(tipoProducto.getTipoProductoDescripcion()));
        if (tipoProducto.getTipoProductoEstado().equals(TipoProducto.ESTADO_ACTIVO)) {
    ...
    private void initComponents() {
        ...
        jLabel1.setText("Datos de Tipo de Productos ");
        ...
        rSLabelImage2.setIcon(new javax.swing.ImageIcon(getClass().getResource("/restaurant/capa5_imagenes/nombretipoproducto.png"))); // NOI18N
        ...
        jPanel2.add(jPanel4, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 60, 320, 290));
    ...
    private void btnGurdarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnGurdarActionPerformed
        tipoProducto.setTipoProductoCodigo(Integer.parseInt(txtCodigo.getText()));
        tipoProducto.setTipoProductoNombre(txtNombre.getText());
        tipoProducto.setTipoProductoDescripcion(txtDescripcion.getText());
        if (cboEstado.getSelectedItem().toString().equals("ACTIVO")) {
            tipoProducto.setTipoProductoEstado(TipoProducto.ESTADO_ACTIVO);
        } else if (cboEstado.getSelectedItem().toString().equals("INACTIVO")) {
            tipoProducto.setTipoProductoEstado(TipoProducto.ESTADO_INACTIVO);
        }
        gestionarTipoProductoServicio = new GestionarTipoProductoServicio();
        int registros_afectados;
        if (tipo_accion == ACCION_CREAR) {
            try {
                registros_afectados = gestionarTipoProductoServicio.crearTipoProducto(tipoProducto);
                if (registros_afectados == 1) {
                    Mensaje.mostrarAfirmacionDeCreacion(this);
                    DesktopNotify.showDesktopMessage("FiveCod software", "Usted Acaba crear un nuevo Tipo de Producto", 7);
            ...
            try {
                registros_afectados = gestionarTipoProductoServicio.modificarTipoProducto(tipoProducto);
                if (registros_afectados == 1) {
